...
append Libero_project_name "_EVMPF300_DEV_KIT_ES"
append project_dir "_EVMPF300_DEV_KIT_ES"
append project_folder_name "_EVMPF300_DEV_KIT_ES"
...
if {[file exists $project_dir] == 1} then {
	...
	new_project -location $project_dir -name $Libero_project_name -project_description {} -block_mode 0 -standalone_peripheral_initialization 0 -instantiate_in_smartdesign 1 -ondemand_build_dh 1 -hdl {VERILOG} -family {PolarFire} -die {MPF300T_ES} -package {FCG1152} -speed {-1} -die_voltage {1.0} -part_range {IND} -adv_options {IO_DEFT_STD:LVCMOS 1.8V} -adv_options {RESTRICTPROBEPINS:1} -adv_options {RESTRICTSPIPINS:0} -adv_options {TEMPR:IND} -adv_options {UNUSED_MSS_IO_RESISTOR_PULL:None} -adv_options {VCCI_1.2_VOLTR:EXT} -adv_options {VCCI_1.5_VOLTR:EXT} -adv_options {VCCI_1.8_VOLTR:EXT} -adv_options {VCCI_2.5_VOLTR:EXT} -adv_options {VCCI_3.3_VOLTR:EXT} -adv_options {VOLTR:IND} 
	source ./EVMPF300_DEV_KIT_ES/import_component_and_constraints_evpf_dev.tcl
	save_project
}
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	run_tool -name {VERIFYTIMING}
    export_prog_job \
        -job_file_name {EVMPF300_DEV_KIT_ES-ProgramFile} \
        -export_dir {./} \
