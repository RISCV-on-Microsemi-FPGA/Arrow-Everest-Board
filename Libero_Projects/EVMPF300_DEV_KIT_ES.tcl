...
append Libero_project_name_ahb "_EVMPF300_DEV_KIT_ES"
append project_dir_ahb "_EVMPF300_DEV_KIT_ES"
append project_folder_name_ahb "_EVMPF300_DEV_KIT_ES"
...
append Libero_project_name_axi "_EVMPF300_DEV_KIT_ES"
append project_dir_axi "_EVMPF300_DEV_KIT_ES"
append project_folder_name_axi "_EVMPF300_DEV_KIT_ES"
...
proc create_new_project_label_ahb { }\
{
	...
	puts "--------MiV_RV32IMA_L1_AHB_BaseDesign_EVMPF300_EVAL_KIT------------------"
    puts "-------------------------------------------------------------------------"
	puts "-------------------------------------------------------------------------"
}
...
proc create_new_project_label_axi { }\
{
    ...
    puts "-------------------------------------------------------------------------"
    puts "-----------------------CREATING NEW PROJECT------------------------------"
	puts "--------MiV_RV32IMA_L1_AXI_BaseDesign_EVMPF300_EVAL_KIT------------------"
    puts "-------------------------------------------------------------------------"
	puts "-------------------------------------------------------------------------"
}
...
if {"$target" == "AHB"} then {
	if {[file exists $project_dir_ahb] == 1} then {
		...
		source ./import/components/AHB/import_component_and_constraints_evmpf300_es_ahb.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
	...
	if {[file exists $project_dir_axi] == 1} then {
		...
		source ./import/components/AXI/import_component_and_constraints_evmpf300_es_axi.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
...
# ##
	...
	if {"$target" == "AHB"} then {
		export_prog_job \
			-job_file_name {BaseDesign_EVMPF300-AHB-ProgramFile} \
		...
		export_prog_job \
			-job_file_name {BaseDesign_EVMPF300-AXI-ProgramFile} \
			-export_dir {./../FlashPro_Express_Projects/Programming_Files} \
			-bitstream_file_type {TRUSTED_FACILITY} \
			-bitstream_file_components {}
		save_project
